        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white border border-gray-100 rounded-2xl p-6 sticky top-24">
                    <div class="text-center mb-6">
                        <div class="w-24 h-24 bg-gradient-to-br from-emerald-400 to-green-600 rounded-3xl flex items-center justify-center text-white font-black text-4xl mx-auto mb-4 shadow-lg">
                            {{ substr($konsultan->user->name, 0, 1) }}
                        </div>
                        <h1 class="text-xl font-extrabold text-gray-900">{{ $konsultan->user->name }}</h1>
                        <span class="inline-block bg-emerald-100 text-emerald-700 text-sm font-semibold px-3 py-1 rounded-full mt-2">
                        ...
                        <div class="flex items-center justify-center gap-2 mt-3 text-sm">
                            ...
                            <span class="text-gray-400">({{ $konsultan->total_konsultasi }} sesi)</span>
                        </div>
                    </div>

                    <div class="border-t border-gray-100 pt-4 space-y-3 text-sm">
                        <div class="flex justify-between">
                            <span class="text-gray-500">Harga Sesi</span>
                            <span class="font-bold text-gray-900">Rp {{ number_format($konsultan->harga_per_sesi, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Durasi</span>
                            <span class="font-semibold">{{ $konsultan->durasi_menit }} menit</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Total Konsultasi</span>
                            <span class="font-semibold">{{ $konsultan->total_konsultasi }}</span>
                        </div>
                    </div>

                    @if($konsultan->bio)
                    <div class="border-t border-gray-100 pt-4 mt-4">
                        <h3 class="font-semibold text-gray-700 mb-2 text-sm">Tentang</h3>
                        <p class="text-gray-500 text-sm leading-relaxed">{{ $konsultan->bio }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white border border-gray-100 rounded-2xl p-6">
                    <h2 class="text-xl font-bold text-gray-900 mb-6">Buat Jadwal Konsultasi</h2>

                    @guest
                    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center mb-6">
                        <p class="text-amber-800 font-medium mb-3">Masuk terlebih dahulu untuk booking</p>
                        <a href="{{ route('login') }}" class="bg-emerald-600 text-white font-semibold px-6 py-2.5 rounded-xl text-sm hover:bg-emerald-700 transition-colors">
                            Masuk Sekarang
                        </a>
                    </div>
                    @endguest
                    ...
                    <form action="{{ route('konsultasi.store', $konsultan) }}" method="POST" class="space-y-5">
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                            <div class="flex justify-between text-sm mb-2">
                                ...
                                <span class="font-semibold">Rp {{ number_format($konsultan->harga_per_sesi, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-sm mb-3">
                                <span class="text-gray-600">Biaya admin (~2%)</span>
                                <span class="font-semibold">Rp ~{{ number_format(max($konsultan->harga_per_sesi * 0.02, 2000), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between font-bold border-t border-gray-200 pt-2">
                                <span>Total</span>
                                <span class="text-emerald-700">≈ Rp {{ number_format($konsultan->harga_per_sesi + max($konsultan->harga_per_sesi * 0.02, 2000), 0, ',', '.') }}</span>
                            </div>
                        </div>

                        <p class="text-xs text-gray-400">
                            💳 Pembayaran via <strong>QRIS</strong> atau <strong>Transfer Virtual Account</strong>. Link meeting dikirim ke WhatsApp & Email setelah pembayaran dikonfirmasi.
                        </p>

                        @auth
                        <button type="submit" id="btn-lanjut-bayar"
                            class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl transition-colors shadow-md text-base">
                            Lanjut ke Pembayaran →
                        </button>
                        @else
                        <a href="{{ route('login') }}" class="block w-full text-center bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl transition-colors shadow-md">
                            Masuk untuk Booking
                        </a>
                        @endauth
                    </form>
